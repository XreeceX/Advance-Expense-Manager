...
def load_expenses(user_id: str, month: Optional[str] = None, category: Optional[str] = None) -> pd.DataFrame:
    ...
    if month:
        # dt is stored as 'YYYY-MM-DD' string; filter by prefix
        q["dt"] = {"$regex": f"^{month}"}
    if category:
        q["category"] = category
...
def page_add(user):
    ...
    with st.form("add_form", clear_on_submit=True):
        c1, c2 = st.columns(2)
        dt_val = c1.date_input("Date", value=date.today())
    ...
    if submitted:
        add_expense(user["id"], dt_val.isoformat(), category, description, amount, payment)
        st.success("Added!")
